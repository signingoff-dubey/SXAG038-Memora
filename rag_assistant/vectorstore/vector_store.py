...
class ChromaVectorStore(BaseVectorStore):
    ...
    def __init__(
        ...
        default_persist_dir = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__), "..", "..", "backend", "data", "chroma"
            )
        )
        self.persist_directory = persist_directory or os.getenv(
            "VECTOR_STORE_PATH", default_persist_dir
        )
            ...
            if results and results.get("documents"):
                for i, doc_content in enumerate(results["documents"][0]):
                    ...
                    search_results.append(
                        SearchResult(document=document, score=score, metadata=metadata)
                    )
...
class FAISSVectorStore(BaseVectorStore):
    ...
    def __init__(
        self,
        collection_name: str = "vulnerability_docs",
        ...
        """
        super().__init__(collection_name)
        default_path = Path(tempfile.gettempdir()) / f"faiss_{collection_name}"
    ...
    def search(
        ...
        try:
            ...
            distances, indices = self.index.search(
                query, min(top_k, len(self._doc_ids))
            )
