...
async def _merge_similar(user_id: str, db: AsyncSession):
    ...
    for cluster_indices in clusters:
        ...
        newest_ts = max(
            (m.last_accessed_at or _utcnow()) for m in cluster_memories
        )
        ...
        vector_store.update_memory(
            ...
            {"user_id": user_id, "importance": best_importance,
             "created_at": keeper.created_at.isoformat() if keeper.created_at else ""},
        )
